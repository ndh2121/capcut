import React from "react";
import "./SmartTool.css";

const SmartTool = () => {
    return (
        <section className="pc_landing_smart_tool">
            <div className="ojcowekkr">
                <h2 className="owjdowijrlk">
                    <span>Smart tools. Highly efficient.</span>
                </h2>
                <p className="ojwirwowwaowir">
                    Smart tools to help you create videos easier
                </p>
            </div>
            <div className="kopdwkwlglkfw">
                <div className="kopdwkwlglkfw_item lsjkcowklc_active">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/b7ba2555388b4d9595d5ecda9c6da797~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>Remove background</span>
                    </div>
                </div>
                <div className="kopdwkwlglkfw_item">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/fe0bff2d8d2143378119520062741d35~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>Image Enlarger</span>
                    </div>
                </div>
                <div className="kopdwkwlglkfw_item">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/01b6834f026a4753810a702b823259a6~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>AI color correction</span>
                    </div>
                </div>
                <div className="kopdwkwlglkfw_item">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/45d646ea4f3240a79d475979b4a2498c~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>Restore old photos</span>
                    </div>
                </div>
                <div className="kopdwkwlglkfw_item">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/6e75afe2e01d45dfadc35864e1153425~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>Photo color changer</span>
                    </div>
                </div>
                <div className="kopdwkwlglkfw_item">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/fe3166bfe473422dbe2afdcce722ea89~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>Portrait Maker</span>
                    </div>
                </div>
                <div className="kopdwkwlglkfw_item">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/c9e9e30673ff4f849ca7f95cb6701a90~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>Resize video</span>
                    </div>
                </div>
                <div className="kopdwkwlglkfw_item">
                    <div className="img_ojowkwd">
                        <img
                            src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/1c2ab42ad5d346d79bc3cfe482e3f337~tplv-6rr7idwo9f-image.image"
                            alt="Photo"
                        />
                    </div>
                    <div className="name_ojowkwd">
                        <span>Video Resolution Enhancer</span>
                    </div>
                </div>
            </div>
            <div className="kojkkclsk">
                <div className="jkjocjslk">
                    <section className="lsjowirrsa">
                        <div className="ojcwlkc">
                            <h2>Remove background</h2>
                            <p>
                                Remove portrait video background accurately and
                                replace background with uploaded photo or change
                                background color.
                            </p>
                            <button>Try for free</button>
                        </div>
                        <div className="ojwrlksjlaowi">
                            <div className="ksjhcosufjskl">
                                <video
                                    loop
                                    autoPlay
                                    muted
                                    preload="auto"
                                    src="https://p16-capcut-va.ibyteimg.com/tos-maliva-i-6rr7idwo9f-us/4ac2c75058fe48b1b0f9375436a98416~tplv-6rr7idwo9f-image.image"
                                ></video>
                            </div>
                        </div>
                    </section>
                </div>
            </div>
        </section>
    );
};

export default SmartTool;
